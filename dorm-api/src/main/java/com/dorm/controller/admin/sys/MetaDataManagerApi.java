...
public class MetaDataManagerApi {
    ...
    public JsonResult buildingList(PageView<Building> page, String position) {
        String kw = position;
...
//            criteria.andLike("roomNo", "%"+roomNo+"%");
            criteria.andEqualTo("roomNo", roomNo);
        }
        if(buildingId != null){
    ...
    public JsonResult updateDormStatus(Integer buildingId, Integer dormitoryId, String remark, Integer status) {
        JsonResult result = new JsonResult(true, "0", "查询成功");
        // 创建Example对象
        Example example = new Example(Dormitory.class);
        Example.Criteria criteria = example.createCriteria();
        ...
        // 根据buildingId和dormitoryId查询对应的Dormitory数据
        Dormitory dorm = dormitoryMapper.selectOneByExample(example);
