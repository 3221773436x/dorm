        ...
        try {
            ...
            if(occ.getOccupationId()==null){
                ...
                if(existList!=null&&existList.size()>0){
                    ...
                    Dormitory dormTemp = dormMapper.selectByPrimaryKey(occExist.getDormId());
                    dormTemp.setOccupied(dormTemp.getOccupied()-1);

                    dormMapper.updateByPrimaryKeySelective(dormTemp);
                }
                occ.setCheckInTime(new Date());
                occ.setOperatorId(manager.getAdminId());
                occupationMapper.insertSelective(occ);
                //将入住的宿舍对应的已入住人数加1
                Dormitory dorm1 = dormMapper.selectByPrimaryKey(occ.getDormId());
                dorm1.setOccupied(dorm1.getOccupied() + 1);
                dormMapper.updateByPrimaryKeySelective(dorm1);
            }else{
